...
final class OnboardingViewModel: ObservableObject {
    ...
    private let getPreferOptionsUseCase: GetPreferOptionsUseCase
    
    init(getPreferOptionsUseCase: GetPreferOptionsUseCase) {
        self.getPreferOptionsUseCase = getPreferOptionsUseCase
        Task {
    ...
    func fetchPreferences() async {
        do {
            allOptions = try await getPreferOptionsUseCase.execute()
            print(allOptions)
        } catch {
            print("\(error)")
        }
